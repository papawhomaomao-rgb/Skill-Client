...
const DEMO_DEVICE = {
  ok: true, status: "pending", match_code: "K7-F92",
  hostname: "Vens-PC", os: "Windows 11 23H2", version: "1.0.4",
...
function LinkPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${LINK_API}/auth/device/${encodeURIComponent(DEVICE_CODE)}`);
      const d = await r.json().catch(() => null);
      if (!r.ok || !d || !d.ok) { setPhase("error"); setError(d?.error || "expired"); return; }
      if (d.status === "expired" || d.status === "denied") { setPhase(d.status === "denied" ? "denied" : "error"); setError("expired"); return; }
      setDevice(d);
    } catch (e) {
  ...
  const decide = async (approve) => {
    ...
    try {
      ...
      const d = await r.json().catch(() => null);
      if (r.ok && d && d.ok) setPhase(approve ? "approved" : "denied");
      else setError(d?.error || "Could not complete that. Try restarting the launcher.");
    } catch (e) {
  ...
  return (
    <Shell>
      ...
      <div style={{ marginBottom: 22 }}>
        {device.hostname && <Row k="Computer" v={device.hostname} />}
        {device.os && <Row k="System" v={device.os} />}
        <Row k="Client" v={`Skilled ${device.version}`} />
        {device.expires_at && (
